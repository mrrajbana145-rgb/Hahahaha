...
async def help_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(
        "🆘 **HELP & SUPPORT**\n\n"
        "📞 Support: @Tigertransportbot\n"
        "📢 Channel: @callhistry\n\n"
        "💳 **Payment Issues:**\n"
        "• Contact support with UTR\n"
...
async def button_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Check Balance
        ...
        await query.edit_message_text(
            ...
            f"• Requests: {user['history_requests']}\n\n"
            f"💳 **Deposit Range:**\n"
            f"Min: ₹600 | Max: ₹10,000\n\n"
            f"👥 **Referral Progress:**\n"
    ...
    # Referral Program
        ...
        await query.edit_message_text(
            ...
            f"• Each referral: 1 credit\n"
            f"• 100 credits: 50% discount\n\n"
        ...
        if plan_key in plan_map:
            ...
            if user["balance"] < plan["price"] and user_id != OWNER_ID:
                await query.edit_message_text(
                    ...
                    f"Required: ₹{plan['price']}\n"
                    f"Your balance: ₹{user['balance']}\n\n"
                    "Please add funds first.",
                    reply_markup=InlineKeyboardMarkup([[
                        InlineKeyboardButton("💳 Add Funds", callback_data="add_funds"),
                        InlineKeyboardButton("🔙 Back", callback_data="history_rc" if "rc" in plan_key else "call_history")
                    ]]),
                    parse_mode='Markdown'
                )
                return
    ...
    # Admin panel options
        ...
        if data == "admin_panel":
            ...
            for uid, user_data in all_users.items():
                user_list.append(f"ID: {uid} | Balance: ₹{user_data['balance']}")
...
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Handle UTR input
        ...
        if len(text) < 12:
            await update.message.reply_text("❌ UTR must be 12+ characters")
            return
        ...
        try:
            await context.bot.send_message(
                ...
                f"Amount: ₹{context.user_data.get('deposit_amount', 0)}\n"
                f"UTR: {text}\n"
...
📋 Details:
• Target: {text}
• Period: {context.user_data.get('plan_days', 30)} days
